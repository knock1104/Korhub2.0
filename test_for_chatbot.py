...
>>>>>>> 9a9ccc8a071cbec2a74fe92e9ca1d4251272864d
import parselmouth
import numpy as np
...
# 모음별 포먼트 범위 설정_for man
# 성별과 국적 그리고 연령에 따라 포먼트 적용
...
if vowel:
    ...
    if gender == '남':
        ...
        #image_file_path = f'/Users/hyunsupark/Desktop/Metacom/Data/audio_file/test용 음성/이미지/남성/{vowel}.png'
    else:  # '여자'의 경우
        audio_file_path = f'/Users/hyunsupark/Desktop/Metacom/Data/audio_file/test용 음성/여성/{vowel}.wav'
        #image_file_path = f'/Users/hyunsupark/Desktop/Metacom/Data/audio_file/test용 음성/이미지/여성/{vowel}.png'
    
    # 가이드 음성 재생
    st.audio(audio_file_path, format='audio/wav')

    st.subheader('가이드 음성 파형')
    # 가이드 이미지 제공
    #st.image(image_file_path, caption=f"'{vowel}'의 스펙트로그램", use_column_width=True)
...
if st.button('Start Recording'):
    ...
    st.write("가이드 음성파형")    
    x, _ = librosa.load(audio_file_path, sr=fs)
    ...
    #학습자 파형
    st.write("학습자 음성파형")    
    x, _ = librosa.load(audio_file, sr=fs)
    y = librosa.stft(x, n_fft=128, hop_length=64, win_length=128)
